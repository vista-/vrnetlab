...
class OpenBSD_vm(vrnetlab.VM):
    def __init__(
        ...
        super(OpenBSD_vm, self).__init__(
...
class OpenBSD(vrnetlab.VR):
    def __init__(self, hostname, username, password, nics, conn_mode):
        super(OpenBSD, self).__init__(username, password)
        self.vms = [OpenBSD_vm(hostname, username, password, nics, conn_mode)]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hostname", default="openbsd", help="VM Hostname")
    # if nics >= 18, OpenBSD 7.3 start to produce errors like this: "failed to allocate interrupt slot for PIC msix pin -2147375103"
    parser.add_argument("--nics", type=int, default=17, help="Number of NICS")
    parser.add_argument(
        "--connection-mode",
        default="tc",
